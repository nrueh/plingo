...
if __name__ == '__main__':
    ...
    our_probs = our_data[1:, 1].astype(np.int32)
    our_times = our_data[1:, 2].astype(np.float32)
    ...
    az_probs = az_data[1:, 1].astype(np.int32)
    az_times = az_data[1:, 2].astype(np.float32)
    ...
    #plt.yscale('log')
    plt.ylim([-0.5, 11])
    plt.title(f'Comparison of runtimes for p=1.0')
