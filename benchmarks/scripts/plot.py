...
def clean_df(df):
    ...
    new_cols = [p for c,p in list(itertools.product(all_constraints, all_stats))]
    ...
    # required_colums = ["instance-name"] +  all_constraints 
...
# ------- Aux for tex output
def csv2textable(df,ins,approaches,spetial_words,base_headers = [""],caption=None,limitter='stats'):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--approach", type=str, action='append',
            help="Approach to be plotted folowed by name devided by a dot. Can pass multiple",required=True)
    parser.add_argument("--dom",type=str, default='',
            help="Name of domain" )
    parser.add_argument("--opt", type=str, default='mpe',
    ...
    for instance in instances:
        ...
        for s in stats:
            ...
            for a,n in approaches:
                ...
                rows.append(row)
        df_row = pd.DataFrame(rows, columns=["Stat"]+approaches)
        ...
        dfs_per_instance[instance]= df_row
    ...
    if args.type == "table":
        ...
        for ins, df in dfs_per_instance.items():
            ...
            tex_table = csv2textable(df,ins,approaches,spetial_words={s:'stats' for s in stats})
    ...
    # Nico: 
    # You can edit this part, here there is a data frame per instance, perhaps we need to reorder them differently before lines 255 to 285
    elif args.type == "bar":
        approaches_colors = {
            "plingo":"#C8F69B",
            "problog":"#FFB1AF",
        ...
        colors = [approaches_colors[a] for a,n in approaches]
        print(dfs_per_instance)
        # -------- Plot
        for ins, df in dfs_per_instance.items():
            ...
            for i, s in enumerate(plotting_stats):
                ...
                stats_row.plot(x="Stat", y=approaches, kind="bar", color=colors)
