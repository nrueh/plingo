...
python scripts/plot.py --dom grid --opt exact --prefix 'runtime-all' --x '#Instances solved' --y 'Runtime (s)'  -a plingo.bm_plog -a problog.bm -a plog.bm -a plog.bm_dco -a azreasoners.bm -a plingo.bm_b100000 -a plingo.bm_b1000000 -t cactus --title="Grid (Marginal inference)" --range-from=5 --range-every=5
...
python scripts/plot.py --dom grid --opt sample --prefix 'prob' -t prob -a plingo.bm_b10 -a plingo.bm_b100 -a plingo.bm_b1000 -a plingo.bm_b10000 -a plingo.bm_b100000 -a plingo.bm_b1000000 -a problog.bm --x 'True probability' --y 'Approximate Probability' --title="Grid (Marginal inference aproximate)"
...
python scripts/plot.py --opt exact --y 'Runtime (s)' --x '#Days' --prefix 'runtime' --dom squirrel -a plog.bm -a plog.bm_dco -a plingo.bm -t line --title 'Squirrel (Marginal inference)' --range-from=4 --range-every=5
...
python scripts/plot.py --opt mpe --prefix 'runtime' --dom alzheimer_problog -a plingo.bm -a azreasoners.bm -t line --y 'Runtime (s)' --title="Alzhaimer (MAP inference)" --x 'Instance size' --range-from=5 --range-every=5
...
python scripts/plot.py --opt mpe --y 'Runtime (s)' --prefix 'runtime' --dom smokers -a plingo.bm -a azreasoners.bm -t line --x 'Instance size' --title="Smokers (MAP inference)"
