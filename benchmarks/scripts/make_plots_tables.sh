...
python scripts/plot.py --dom grid --opt exact --prefix 'runtime-all' --x '#Instances solved' --y 'Runtime (s)'  -a plingo.bm_plog -a problog.bm -a plog.bm -a plog.bm_dco -a azreasoners.bm  -a plingo-problog.bm  -a plingo.bm_b100000 -a plingo.bm_b1000000 -t cactus --title="Grid (Marginal inference)" --range "5,40,5"
